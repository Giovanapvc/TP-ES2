...
def choose(prompt: str, options: dict):
    """Menu simples com teclas de atalho."""
    while True:
        print(prompt)
...
def artist_dashboard(store, artist: Artist):
    while True:
        act = choose(
            ...
            {"S": "Add música", "E": "Editar música", "V": "Stats", "B": "Voltar"},
        )
        if act in ("B", CANCEL_KEY):
            break
        if act == "S":
            title = inp("Título")
            if title is None:
...
def edit_song(store, artist, song):
    while True:
        act = choose(
            f"Editar {song.title}",
            {"T": "Título", "D": "Descrição", "G": "Gênero", "B": "Voltar"},
        )
        if act in ("B", CANCEL_KEY):
            break
        if act == "T":
            new = inp("Novo título")
            if new is not None:
                service.edit_song(store, artist.id, song.id, title=new)
        elif act == "D":
            new = inp("Nova descrição")
            if new is not None:
                service.edit_song(store, artist.id, song.id, description=new)
        elif act == "G":
            g = choose("Gênero", {g.upper(): g for g in GENRES})
            if g not in (CANCEL_KEY, None):
                service.edit_song(store, artist.id, song.id, genre=g)
        print("Atualizado.\n")
...
def user_flow(store):
    ...
    while True:
        ...
        if act == "G":
            ...
            browse_songs(
                [s for a in store.values() for s in a.songs if s.id in liked], liked, store
            )
        elif act == "A":
            artists = service.artists_sorted(store)
            for i, a in enumerate(artists, 1):
                print(f"{i}. {a.name} ({a.likes})")
            val = inp("Número para curtir")
            if val is None or not val.isdigit():
                continue
            n = int(val)
            if 1 <= n <= len(artists):
                service.like_artist(store, artists[n - 1].id)
                print("Curtido.\n")
                pause()
        elif act == "O":
            while True:
                ...
                if song:
                    show_song(song, store)
                    opinion_flow(store, song, liked)
                    break
